...
export default function BinarySampleLoader({ modelShape, onFeatureLoaded, onTenhouUrl }: Props) {
  ...
  const scan = async () => {
    setBusy(true)
    try {
      const response = await fetch(`${API_BASE_URL}/api/binary_samples/files?directory=${encodeURIComponent(directory)}`)
  ...
  const load = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/binary_samples/search`, {
