...
export default function CsvReaderPage({ onBack, onAddModel, onAddTrainingResults }: CsvReaderPageProps) {
  ...
  const loadPage = useCallback(async (sessionId: string, page: number, filterText?: string, includeTotal = false) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/csv_reader/page`, {
        method: 'POST',
  ...
  const openCsvPath = useCallback(async (path: string, restorePage = 0, restoreFilterText = '', pinned = true): Promise<string | null> => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/csv_reader/open`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  }, [activeSession?.sourcePath, imageZoom, pageSize, selectedImageId, sessions, sideMode, sidePaneOpen, sidePaneWidth, sideUrl])
  ...
  const closeSession = useCallback(async (sessionId: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/csv_reader/close`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const openLinkInPane = (value: string) => {
  ...
  useEffect(() => {
    ...
    const loadIndexStatus = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/csv_reader/output_index/status`)
  ...
  const searchOutputIndex = useCallback(async (query = outputQuery) => {
    setIndexBusy(true)
    try {
      const response = await fetch(`${API_BASE_URL}/api/csv_reader/output_index/search?query=${encodeURIComponent(query)}&limit=40`)
      const data: OutputIndexResponse = await response.json()
  ...
  const rebuildOutputIndex = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/csv_reader/output_index/scan`, { method: 'POST' })
      const data: OutputIndexResponse = await response.json()
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'system-ui, sans-serif', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      ...
      {sessions.length > 0 && <div className="csv-session-toolbar">
      ...
      >
        <div style={{ flex: 1, minWidth: 0, overflow: 'auto', border: '1px solid #d0d7de', borderRadius: '6px', backgroundColor: '#fff' }}>
          {!activeSession ? (
                    ...
                    {activeSession.headers.map((header, colIndex) => {
                      ...
                      return (
                        <td
                          ...
                          onDoubleClick={() => isLink && openLinkInPane(value)}
                          onMouseDown={event => {
        ...
        {sidePaneOpen && (
          <>
            ...
            <div style={{ padding: '8px', borderBottom: '1px solid #d0d7de', display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {sideMode === 'web' ? (
                <>
                  <input value={sideUrl} onChange={e => setSideUrl(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') setSideUrl(normalizeUrl(sideUrl)) }} style={{ flex: 1, minWidth: '180px' }} />
